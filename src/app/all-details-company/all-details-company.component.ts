...
export class AllDetailsCompanyComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    //שליפת ערים
    this.cityService.GatAllCity().subscribe(data => {
      this.cityService.listCity = data
      this.sendingCompanyService.newCompany.listCitiesCompany?.map(x => {
        let c = data.find(y => y.CityID == x.CityID)
        if (c)
          x.nameCity = c.CityName
      })
    });
    ...
    //אם זה עדכון
      ...
      for (let d = 0; d < this.sendingCompanyService.newCompany.listBusinessDays?.length; d++) {
        ...
        if (this.di > -1) {
          ...
          //שליפת יום עסקים          
          let busDay = this.sendingCompanyService.newCompany.listBusinessDays.filter(x => x.Day == d_checked.numDay)[0]
  ...
  deleteCity(CitiesCompanyID: number) {
    debugger
    this.cityService.GetRemoveCitiesCompany(CitiesCompanyID).subscribe(data => {
      this.cityService.listCity = data
      // this.sendingCompanyService.newCompany.listCitiesCompany?.map(x => {
      //   let c = data.findIndex(y => y.nameCity == x.CityID)
      //   if (c!=-1)
      //     this.cityService.listCity.splice(c, 1)
      // })
    })}
